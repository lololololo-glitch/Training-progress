...
export default function App() {
  ...
  const saveWorkout = () => {
    ...
    alert("האימון נשמר! עכשיו תוכל לראות מטרות בטאב המטרות.");
  };
  ...
  return (
    <div style={{ direction: 'rtl', backgroundColor: '#050505', minHeight: '100vh', color: '#fff', padding: '20px', fontFamily: 'sans-serif' }}>
      ...
      {/* תוכן הטאבים */}
      {activeTab === 0 && <div style={styles.card}><h2>יומן פעילויות</h2><p>כאן תוסיף אירועים בקרוב...</p></div>}
      
      {activeTab === 1 && (
        <div style={styles.card}>
          <h2>המטרות שלי</h2>
          {sessions.length === 0 ? <p>עדיין אין נתונים. בצע אימון ראשון!</p> : 
           <p>המשקל האחרון היה: {sessions[0][0].weight} ק"ג. המטרה הבאה: {parseFloat(sessions[0][0].weight || 0) + 2.5} ק"ג.</p>}
        </div>
      )}
...
const styles = {
  card: { background: '#111', padding: '20px', borderRadius: '20px', border: '1px solid #333' },
